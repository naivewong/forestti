...
// Write mode, tail_count
BitStream::BitStream(int size, int header_size)
    : stream(size + header_size, 0),
      head_count(0),
      tail_count(0),
      vector_mode(true),
      index(0),
      end(size + header_size),
...
uint8_t *BitStream::bytes() { return &(stream[0]) + header_size; }

const uint8_t *BitStream::bytes_ptr() const {
  if (vector_mode)
    return &(stream[0]) + header_size;
  else
    return stream_ptr;
}
