...
std::pair<std::shared_ptr<chunk::ChunkInterface>, error::Error> merge_chunks(
    const std::shared_ptr<chunk::ChunkInterface> &c1,
    const std::shared_ptr<chunk::ChunkInterface> &c2);
// error::Error merge_group_chunks(
// 		const std::shared_ptr<querier::GroupChunkSeriesMeta> &gcsm,
// 		const std::deque<std::deque<std::shared_ptr<chunk::ChunkMeta>>>
// &chunks, 		int start, int end);
...
// merge_overlapping_group_chunks(
// 		const std::deque<std::deque<std::shared_ptr<chunk::ChunkMeta>>>
// &chunks);
