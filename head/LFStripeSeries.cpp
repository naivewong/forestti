...
void LFStripeSeries::read_lock(int pos) {
  ...
  while (b) {
    ...
    if (locks[pos].compare_exchange_weak(x, x + 1)) b = false;
  }
}

void LFStripeSeries::read_unlock(int pos) { --locks[pos]; }
...
void LFStripeSeries::write_lock(int pos) {
  ...
  while (b) {
    ...
    if (locks[pos].compare_exchange_weak(x, -1)) b = false;
  }
}

void LFStripeSeries::write_unlock(int pos) { ++locks[pos]; }
