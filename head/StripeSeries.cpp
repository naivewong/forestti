...
// std::pair<std::unordered_set<uint64_t>, int> StripeSeries::old_gc(
//     int64_t min_time) {
...
//   // Run through all series and truncate old chunks. Mark those with no
//   // chunks left as deleted and store their ID.
//   std::unordered_set<uint64_t> rm_series, tmp_rm_series;
//   int rm_chunks = 0, tmp_rm_chunks = 0;
//   for (int i = 0; i < STRIPE_SIZE; ++i) {
//     {
//       tmp_rm_series.clear();
//       tmp_rm_chunks = 0;
//       base::PadRWLockGuard lock_i(locks[i], 1);
//       hashes[i].gc(min_time, tmp_rm_series, tmp_rm_chunks);
//     }
