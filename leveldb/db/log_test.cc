...
TEST_F(LogRecordTest, TestRandom1) {
  Env* env = Env::Default();
  name = "head.log";
  boost::filesystem::remove(name);
  std::vector<uint64_t> offsets;
  {
    ...
    RandomRWFile* rf;
    ASSERT_TRUE(env->NewRandomRWFile(name, &rf).ok());
    RandomWriter rw(rf);
    RandomReader r(rf);
    ...
    std::string scratch;
    for (int i = 0; i < 1000; i++) {
      ASSERT_TRUE(r.ReadRecord(offsets[i], &record, &scratch));
      std::vector<RefSeries> rs;
      ASSERT_TRUE(series(record, &rs));
      ASSERT_EQ(1, rs.size());
      ASSERT_EQ(i, rs[0].ref);
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      r.log_clean_txn = i;
      std::string record = series({r});
      ASSERT_TRUE(rw.AddRecord(offsets[i], record).ok());
    }
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      ASSERT_EQ(1, rs.size());
      ASSERT_EQ(i, rs[0].ref);
      ASSERT_EQ(i, rs[0].flushed_txn);
      ASSERT_EQ(i, rs[0].log_clean_txn);
      ASSERT_EQ(lset, rs[0].lset);
    }
...

  {
    ...
    while (r.ReadRecord(&record, &scratch)) {
      std::vector<RefSeries> rs;
      ASSERT_TRUE(series(record, &rs));
      ASSERT_EQ(offsets[i], r.LastRecordOffset());
      i++;
    }
    ASSERT_EQ(1000, i);
    delete sf;
