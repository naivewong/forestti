...
class LEVELDB_EXPORT Env {
 public:
  ...
  virtual std::string RemapFilename(const std::string& logical_path) const {
    return logical_path;
  }

  virtual Env* GetBaseEnv() { return this; }
  ...
  virtual Status CreateDirIfMissing(const std::string& d) {
    return Status::OK();
  }

  virtual bool DirExists(const std::string& dirname) { return false; }
  ...
  virtual Status PrefetchFile(const std::string&, bool) { return Status::OK(); }
};
...
class LEVELDB_EXPORT RandomAccessFile {
 public:
  ...
  virtual RandomAccessType Type() const { return kEmpty; }
...
class LEVELDB_EXPORT Logger {
 ...
 protected:
  virtual Status CloseImpl() { return Status::NotSupported(); }
  bool closed_;
...
class LEVELDB_EXPORT EnvWrapper : public Env {
 public:
  ...
  Status NewStringWritableFile(
      const std::string& f, WritableFile** r,
      const EnvOptions& options = EnvOptions()) override {
    return target_->NewStringWritableFile(f, r, options);
  }
  Status NewAppendableFile(const std::string& f, WritableFile** r,
                           const EnvOptions& options = EnvOptions()) override {
    return target_->NewAppendableFile(f, r, options);
  }
