...
MergedIDSeriesSet::MergedIDSeriesSet(const std::shared_ptr<SeriesSets> &ss)
    : ss(ss), series(new Series()), err_(false) {
  // To move one step for each SeriesInterface.
  ss->next();
  if (ss->empty()) {
...
bool MergedIDSeriesSet::next_helper() const {
  // To move one step for the former SeriesInterface s.
  ss->next(id);
  ...
  id.push_back(0);
  for (int i = 1; i < ss->size(); i++) {
    if (ss->at(i)->at()->tsid() < ss->at(id.front())->at()->tsid()) {
      id.clear();
      id.push_back(i);
    } else if (ss->at(i)->at()->tsid() == ss->at(id.front())->at()->tsid()) {
      id.push_back(i);
    }
  }
  for (int i : id) series->push_back(ss->at(i)->at());
...
bool MergedIDSeriesSet::next() const {
  if (err_) return false;
  return next_helper();
}

std::shared_ptr<SeriesInterface> MergedIDSeriesSet::at() {
  if (id.empty())
    return nullptr;
  else if (id.size() == 1)
    return (*series)[0];
...
bool MergedIDSeriesSet::error() const { return err_; }
