...
void *art_search(const art_tree *t, const unsigned char *key, int key_len) {
  ...
  while (n) {
    ...
    if (n->partial_len) {
      ...
      if (prefix_len != min(MAX_PREFIX_LEN, n->partial_len)) return NULL;
...
void **art_search_ptr(const art_tree *t, const unsigned char *key,
                      int key_len) {
  art_node **child;
  art_node *n = t->root;
  int prefix_len, depth = 0;
...
static void add_child256(art_tree *tree, art_node256 *n, art_node **ref,
...
static void add_child48(art_tree *tree, art_node48 *n, art_node **ref,
                        unsigned char c, void *child) {
...
static void add_child16(art_tree *tree, art_node16 *n, art_node **ref,
                        unsigned char c, void *child) {
...
static void add_child4(art_tree *tree, art_node4 *n, art_node **ref,
                       unsigned char c, void *child) {
...
static void add_child(art_tree *tree, art_node *n, art_node **ref,
                      unsigned char c, void *child) {
  switch (n->type) {
    case NODE4:
...
static void *recursive_insert(art_tree *tree, art_node *n, art_node **ref,
                              const unsigned char *key, int key_len,
                              void *value, int depth, int *old, int replace,
                              art_leaf **new_leaf) {
  // If we are at a NULL node, inject a leaf
  if (!n) {
    art_leaf *leaf = make_leaf(tree, key, key_len, value);
  ...
  if (IS_LEAF(n)) {
    ...
    *ref = (art_node *)new_node;
    add_child4(tree, new_node, ref, l->key[depth + longest_prefix],
               SET_LEAF(l));
    add_child4(tree, new_node, ref, l2->key[depth + longest_prefix],
               SET_LEAF(l2));
    return NULL;
  }
  ...
  if (n->partial_len) {
    ...
    *ref = (art_node *)new_node;
    ...
    if (n->partial_len <= MAX_PREFIX_LEN) {
      ...
      n->partial_len -= (prefix_diff + 1);
      art_leaf *l = minimum(n);
      add_child4(tree, new_node, ref, l->key[depth + prefix_diff], n);
...
RECURSE_SEARCH:;
  ...
  if (child) {
    return recursive_insert(tree, *child, child, key, key_len, value, depth + 1,
                            old, replace, new_leaf);
  }
...
static art_leaf *recursive_delete(art_tree *t, art_node *n, art_node **ref,
                                  const unsigned char *key, int key_len,
                                  int depth) {
  // Search terminated
  if (!n) return NULL;
