#include "third_party/ulid_uint128.hpp"
...
void EncodeTimeNow(ULID &ulid) {
  ...
  EncodeTime(std::chrono::duration_cast<std::chrono::nanoseconds>(
...
bool validate_strict(const std::string &v) {
  if (v.length() != 26 || int(v[0]) > 55 || dec[int(v[0])] == 0xFF ||
      dec[int(v[1])] == 0xFF || dec[int(v[2])] == 0xFF ||
      dec[int(v[3])] == 0xFF || dec[int(v[4])] == 0xFF ||
      dec[int(v[5])] == 0xFF || dec[int(v[6])] == 0xFF ||
      dec[int(v[7])] == 0xFF || dec[int(v[8])] == 0xFF ||
      dec[int(v[9])] == 0xFF || dec[int(v[10])] == 0xFF ||
      dec[int(v[11])] == 0xFF || dec[int(v[12])] == 0xFF ||
      dec[int(v[13])] == 0xFF || dec[int(v[14])] == 0xFF ||
      dec[int(v[15])] == 0xFF || dec[int(v[16])] == 0xFF ||
      dec[int(v[17])] == 0xFF || dec[int(v[18])] == 0xFF ||
      dec[int(v[19])] == 0xFF || dec[int(v[20])] == 0xFF ||
      dec[int(v[21])] == 0xFF || dec[int(v[22])] == 0xFF ||
      dec[int(v[23])] == 0xFF || dec[int(v[24])] == 0xFF ||
      dec[int(v[25])] == 0xFF)
    return false;
  return true;
}
...
bool validate_weak(const std::string &v) {
  return v.length() == 26 && int(v[0]) <= 55;
}

// Added by Alec Wang.
bool Validate(const std::string &str, bool strict) {
  if (strict)
    return validate_strict(str);
  else
    return validate_weak(str);
}
