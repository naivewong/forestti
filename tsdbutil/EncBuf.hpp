...
class EncBuf {
 public:
  ...
  EncBuf(int size) : index(0) { b.reserve(size); }
  ...
  const std::vector<uint8_t> &get_vector() { return b; }
  ...
  void put_BE_uint16(int num) {
    if (index + 2 > b.size()) {
      int padding = index + 2 - (int)(b.size());
  ...
  void put_BE_uint32(uint32_t num) {
    if (index + 4 > b.size()) {
      int padding = index + 4 - (int)(b.size());
      // printf("padding:%d\n", padding);
      for (int i = 0; i < padding; i++) b.push_back((uint8_t)(0));
    }
  ...
  void put_BE_uint64(uint64_t num) {
    if (index + 8 > b.size()) {
      int padding = index + 8 - (int)(b.size());
      for (int i = 0; i < padding; i++) b.push_back(0);
    }
  ...
  void put_unsigned_variant(uint64_t num) {
    if (index + base::MAX_VARINT_LEN_64 > b.size()) {
      int padding = index + base::MAX_VARINT_LEN_64 - (int)(b.size());
      for (int i = 0; i < padding; i++) b.push_back(0);
    }
